...
#
#        CUDA_VISIBLE_DEVICES=0 python execute_few_shot_new_align_newscls.py  --few_shot=$few_shot --few_shot_method=$few_shot_method --range=Model/engTonor  --loss_weight=0.2 --target_domain_sim=0.6  --news_cls_iter=1  --random_seed=$random_seed --loss_weight_align=1
#    done
#  done
#done


#_200_4shot _200_2shot
# Adressa -> MIND
...
do
  ...
  do
    for sim in 0.5
    do
      for align_weigh in 1
      do
      for random_seed in 42 0 100 1000 2022
      do
      CUDA_VISIBLE_DEVICES=0 python execute_few_shot_new_align_newscls.py --few_shot=$few_shot  --few_shot_method=$few_shot_method --range=Model/final_data --target_domain_sim=$sim --random_seed=$random_seed --loss_weight=0.2 --news_cls_iter=0
#      CUDA_VISIBLE_DEVICES=0 python execute_few_shot_new_align_newscls.py --few_shot=$few_shot  --few_shot_method=$few_shot_method --range=Model/final_data --target_domain_sim=$sim --random_seed=$random_seed --loss_weight=0.2  --news_cls_iter=1  --loss_weight_align=$align_weigh
    done
    done
    done
  done
